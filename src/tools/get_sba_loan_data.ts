...
interface SbaCkanResponse {
  result?: {
    records?: SbaRawRecord[];
    total?: number;
  };
  success?: boolean;
}

interface SbaRawRecord {
  BorrName?: string;
  BorrZip?: string;
  BorrCity?: string;
  BorrState?: string;
  GrossApproval?: string | number;
  ApprovalDate?: string;
  NaicsCode?: string;
...
export async function getSbaLoanData(params: GetSbaLoanDataParams): Promise<SbaLoanRecord[]> {
  const { zip_code, naics_code, year_range } = params;
  const results: SbaLoanRecord[] = [];
...
async function querySbaResource(
  resourceId: string,
  zipCode: string,
  naicsCode?: string,
  yearRange?: [number, number]
): Promise<SbaLoanRecord[]> {
  // Build filter object
  ...
  const data = await fetchJson<SbaCkanResponse>(url, { maxRetries: 3 });
