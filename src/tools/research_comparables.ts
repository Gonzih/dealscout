...
export async function researchComparables(params: ResearchComparablesParams): Promise<ComparableTransaction[]> {
  const { sector, revenue_range, geography } = params;
  const results: ComparableTransaction[] = [];
  ...
  const filtered = results.filter((r) => {
    if (!r.revenue_range) return true;
...
async function scrapeAxial(sector: string): Promise<ComparableTransaction[]> {
...
function parseAxialHtml(html: string, sector: string): ComparableTransaction[] {
  const results: ComparableTransaction[] = [];
...
async function scrapeBizBuySellSold(
  sector: string,
  geography?: string
...
function parseSoldListings(html: string, sector: string, geography?: string): ComparableTransaction[] {
  const results: ComparableTransaction[] = [];
...
function getHardcodedBenchmarks(
  sector: string,
  revenue_range: [number, number],
  geography?: string
  ...
  const benchmarks: Record<string, ComparableTransaction[]> = {
    hvac: [
      {
