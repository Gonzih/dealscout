...
export async function scoutListings(params) {
    ...
    return results.filter((r) => {
        if (seen.has(r.listing_url))
            return false;
        seen.add(r.listing_url);
        return true;
    });
}
async function scrapeBizBuySell(zip, keyword, maxPrice) {
...
function parseBizBuySellHtml(html, maxPrice) {
    const listings = [];
    ...
    for (const block of listingBlocks) {
        ...
        if (maxPrice && asking_price && asking_price > maxPrice)
            continue;
        // Revenue
...
async function scrapeBizQuest(zip, keyword, maxPrice) {
    const encodedKeyword = encodeURIComponent(keyword);
...
function parseBizQuestHtml(html, maxPrice) {
    const listings = [];
...
//# sourceMappingURL=scout_listings.js.map