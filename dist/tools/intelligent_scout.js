...
export async function intelligentScout(query) {
    ...
    if ((intent.action === "value" || intent.action === "comprehensive") && intent.revenue && intent.sde) {
        ...
        // Structure the deal at mid valuation
        if (intent.action === "comprehensive" || intent.action === "structure") {
    ...
    if (intent.action === "research" || intent.action === "comprehensive") {
        const revenueRange = intent.revenue
            ? [intent.revenue * 0.5, intent.revenue * 2]
            : [500_000, 5_000_000];
        tasks.push(researchComparables({
            sector: intent.sector,
...
function parseQuery(query) {
    ...
    if (q.includes("hvac") || q.includes("heating") || q.includes("cooling") || q.includes("air condition"))
        sector = "hvac";
    else if (q.includes("plumb"))
        sector = "plumbing";
    else if (q.includes("electric"))
        sector = "electrical";
    else if (q.includes("pest") || q.includes("exterminat"))
        sector = "pest_control";
    else if (q.includes("landscape") || q.includes("lawn"))
        sector = "landscaping";
    else if (q.includes("roof"))
        sector = "roofing";
    else if (q.includes("clean") || q.includes("janitor"))
        sector = "cleaning";
    ...
    if (q.includes("nevada") || q.includes(", nv")) {
        ...
        if (zip_codes.length === 0)
            zip_codes.push("89501", "89701", "89431");
    }
    // Extract price
...
function buildSummary(report, intent) {
    const parts = [];
    ...
    if (report.valuation_estimate && typeof report.valuation_estimate === "object") {
        const v = report.valuation_estimate;
    ...
    if (Array.isArray(report.comparables) && report.comparables.length > 0) {
        ...
        const firstComp = report.comparables[0];
...
//# sourceMappingURL=intelligent_scout.js.map